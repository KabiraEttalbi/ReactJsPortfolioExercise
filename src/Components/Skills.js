...
export default function Skills() {
  ...
  const progLang = skills
    .filter((skill) => skill.type === "Programming Languages")
    .map((skill) => <li key={skill.id}>{skill.title}</li>);
  const devTools = skills
    .filter((skill) => skill.type === "Development Tools")
    .map((skill) => <li key={skill.id}>{skill.title}</li>);
  const frameLib = skills
    .filter((skill) => skill.type === "Frameworks and Libraries")
    .map((skill) => <li key={skill.id}>{skill.title}</li>);
  ...
  return (
    <section className="w-full bg-white p-8">
      <div className="container mx-auto max-w-4xl">
        ...
        >
          {showAddSkill ? "Hide Form" : "Add a Skill"}
        </button>
        <br />
        {showAddSkill && <AddSkill setSkills={setSkills} />}
      </div>
    </section>
  );
}
